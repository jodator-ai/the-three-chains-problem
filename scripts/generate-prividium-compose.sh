...
#
#   <output-dir>/docker-compose-l1.yaml             (Anvil L1 + shared postgres)
#   <output-dir>/docker-compose-deps-<id>.yaml      (zksyncos, keycloak, block-explorer per instance;
#                                                    includes docker-compose-l1.yaml)
#   <output-dir>/docker-compose-<id>.yaml           (admin-panel, prividium-api, user-panel per instance;
#                                                    includes docker-compose-deps-<id>.yaml)
#
...
generate_l1() {
  local -r out="$output_dir/docker-compose-l1.yaml"
...
# Included by docker-compose-${chain_id}.yaml via the include: directive.
name: zksync-prividiums

include:
  - docker-compose-l1.yaml
...
generate_prividium_main() {
  ...
  local -r out="$output_dir/docker-compose-${chain_id}.yaml"
...
#
# Includes docker-compose-deps-${chain_id}.yaml (zksyncos, keycloak, block-explorer)
# which in turn includes docker-compose-l1.yaml (Anvil L1, shared postgres).
# A single: docker compose -f docker-compose-${chain_id}.yaml up
# brings up the full stack.
name: zksync-prividiums

include:
  - docker-compose-deps-${chain_id}.yaml
