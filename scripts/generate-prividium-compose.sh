...
#
# Shared infrastructure (Anvil, postgres) lives in docker-compose.l1.yml so
# all instances can reference it without duplication — following the
# local-prividium pattern.
#
...
#   postgres      : 5432  (shared, single instance in docker-compose.l1.yml)
...
generate_prividium() {
  ...
  # Per-instance static config files
...
main() {
  ...
  log "Done. $count prividium compose file(s) + L1+postgres written to: $output_dir"
}
