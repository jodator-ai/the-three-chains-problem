...
#
# All volume paths in generated files are relative to the output directory,
# following the local-prividium pattern.
...
generate_gateway() {
  local -r out="$output_dir/docker-compose.506.yml"
  cat > "$out" <<EOF
# Auto-generated by $SCRIPT_NAME — do not edit manually.
...
services:
  gateway-${GATEWAY_CHAIN_ID}:
    ...
    volumes:
      ...
      - ${rel_configs}/l1/gateway-db.tar.gz:/app/local-chains/${version}/gateway-db.tar.gz:ro
    depends_on:
      l1:
...
generate_chain() {
  ...
  local -r ext_port=$(( 5049 + chain_num ))
  local -r db_vol="zksyncos_${chain_id}_db"
  local -r out="$output_dir/docker-compose.${chain_id}.yml"

  local settle_comment="Settles to: L1"
  local gateway_dep=""
  if [[ "$gateway" == true ]]; then
    settle_comment="Settles to: gateway-${GATEWAY_CHAIN_ID} → L1"
    gateway_dep="
      gateway-${GATEWAY_CHAIN_ID}:
        condition: service_healthy"
  fi
  ...
  cat > "$out" <<EOF
# Auto-generated by $SCRIPT_NAME — do not edit manually.
# Chain ID: $chain_id  |  RPC: localhost:$ext_port  |  $settle_comment
name: zksync-l2s
