...
generate_chain() {
  ...
  cat > "$out" <<EOF
# Auto-generated by $SCRIPT_NAME — do not edit manually.
# Chain ID: $chain_id  |  RPC: localhost:$ext_port$( [[ "$gateway" == true ]] && echo "  |  Settles to: gateway-$GATEWAY_CHAIN_ID" )
name: zksync-l2s
...
main() {
  ...
  local gw_label=""
  if [[ "$gateway" == true ]]; then
    mode_label="gateway settlement (chain 506 → L1)"
    gw_label=" + gateway"
  fi
  log "Done. $count chain compose file(s) + L1${gw_label} written to: $output_dir  [$mode_label]"
}
